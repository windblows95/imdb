...
var (
	...
	titleSeasonCount2RE        = regexp.MustCompile(`<span class="ipc-btn__text">(\d)[^S]*Season</span></a>`)
	...
	titleSeriesIDFromEpisodeRE = regexp.MustCompile(`aria-label="View all episodes" aria-disabled="[^"]*" href="/title/(tt\d+)/episodes/\?ref_=[^"]*">`)
)
...
func (t *Title) Parse(page []byte) error {
	...
	if v.Type == "TVSeries" {
		...
		episodeSeries := titleSeriesIDFromEpisodeRE.FindSubmatch(page)
		if len(episodeSeries) == 2 {
			t.ParentID = string(episodeSeries[1])
		}
